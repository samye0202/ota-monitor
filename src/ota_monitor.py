...
def request_text(url: str) -> str:
    request = urllib.request.Request(url, headers={"User-Agent": USER_AGENT, "Accept": "application/json, application/xml, text/xml, text/html;q=0.9"})
...
def collect_sources() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    errors: list[str] = []
    parsers = {"rss": parse_rss, "json": parse_json_source, "html": parse_html}
    for source in sources:
        try:
            parser = parsers.get(source.get("type", "rss").lower())
            if not parser:
                raise ValueError(f"unsupported source type: {source.get('type')}")
            collected.extend(parser(source, request_text(source["url"])))
...
def run(demo: bool = False) -> None:
    taxonomy = read_json(TAXONOMY_PATH, {"domains": {}, "change_types": {}})
